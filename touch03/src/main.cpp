...
const int nozicaDotika=27; // nožica, na katero je priključen dotik

int vrednostDotika; // spremenljivka, ki bo hranila trenutno vrednost dotika
...
unsigned long zadnjePosiljanje = 0; // spremenljivka, ki bo hranila čas zadnjega poslanega sporočila preko spletnega vtičnika, inicializirana na 0
const int intervalMs = 1; // interval v milisekundah, ki določa, kako pogosto bomo pošiljali sporočila preko spletnega vtičnika, nastavljen na 100 ms (0.1 s)

// 🔥 DODANO: prototip funkcije, da jo lahko onEvent uporabi
...
void setup() {
  ...
  webSocket.onEvent(webSocketEvent); // nastavimo funkcijo webSocketEvent, ki se bo izvajala ob prejemu sporočila preko spletnega vtičnika 8811
...
void webSocketEvent(WStype_t type, uint8_t * payload, size_t length) {
  switch(type) {
    ...
    case WStype_TEXT:
    {
      ...
      //JsonDocument, ki je dovolj velika, da lahko vanj shranimo JSON sporočilo, ki ga prejmemo preko spletnega vtičnika
      JsonDocument doc;
...
void loop() {
  ...
  if (millis() - zadnjePosiljanje >= intervalMs) { // preverimo, če je čas od zadnjega poslanega sporočila preko spletnega vtičnika večji od določenega intervala
    zadnjePosiljanje = millis(); // posodobimo čas zadnjega poslanega sporočila na trenutni čas
    // tu zapišemo kodo, ki jo želimo izvajati periodično, v našem primeru pošiljanje trenutne vrednosti fotoupornika preko spletnega vtičnika 8811 na node.js strežnik
  zadnjePosiljanje = millis(); // posodobimo čas zadnjega poslanega sporočila na trenutni čas
  vrednostDotika=touchRead(nozicaDotika); // preberemo trenutno vrednost dotika in jo shranimo v spremenljivko
  Serial.println(vrednostDotika); // izpišemo trenutno vrednost dotika na serijski monitor
  
  dataString = R"({"tipSporočila":"dotik","pin":27,"vrednost":)";
  dataString = dataString + String (vrednostDotika) + "}"; // sestavimo JSON sporočilo, ki ga bomo poslali preko spletnega vtičnika 8811 na node.js strežnik, v sporočilu je tipSporočila "fotoupornik", pin 27 in vrednost trenutne vrednosti fotoupornika
 ...
 Serial.println(dataString); // izpišemo JSON sporočilo na serijski monitor, da lahko preverimo ali je pravilno sestavljeno
}
